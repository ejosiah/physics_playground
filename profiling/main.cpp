...
static void BM_InterleavedMemoryLayoutSolver(benchmark::State& state){
    ...
    InterleavedMemoryLayoutBasicSolver solver{particles, Bounds, NumParticles, 1.f, 1};
    for(auto _ : state){
        solver.run(timeStep);
    }
}
...
BENCHMARK(BM_SeparateFieldMemoryLayoutSolver);
BENCHMARK(BM_InterleavedMemoryLayoutSolver);
// Run the benchmark
BENCHMARK_MAIN();